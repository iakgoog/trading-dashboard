    ...
    >
      ...
      <div className="flex justify-between items-center mt-0.5">
        <span className="text-slate-500 text-xs tabular-nums">
          Vol:{' '}
          {parseFloat(ticker.volume) >= 1_000_000
            ? `${(parseFloat(ticker.volume) / 1_000_000).toFixed(1)}M`
            : `${(parseFloat(ticker.volume) / 1_000).toFixed(1)}K`}
        </span>
        ...
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-slate-800 z-10">
              <tr>
                <th className="px-3 py-2 text-left font-medium text-slate-400 text-xs uppercase tracking-wider">
                  Symbol
                </th>
                <th className="px-3 py-2 text-right font-medium text-slate-400 text-xs uppercase tracking-wider">
                  Price
                </th>
                <th className="px-3 py-2 text-right font-medium text-slate-400 text-xs uppercase tracking-wider">
                  24h %
                ...
                <th className="px-3 py-2 text-center font-medium text-slate-400 text-xs uppercase tracking-wider">
                  Last 7 Days
                </th>
              </tr>
            </thead>
