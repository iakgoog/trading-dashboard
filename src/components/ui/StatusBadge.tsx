...
export function StatusBadge() {
  ...
  // eslint-disable-next-line react-hooks/purity -- intentional: captures wall-clock time for "Xs ago" display
  const now = Date.now()
  const secondsAgo = lastMessageAt !== null ? Math.floor((now - lastMessageAt) / 1000) : null
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-56 rounded-lg border border-slate-200 bg-white shadow-lg z-10 p-3 text-sm text-slate-700">
          ...
          <p>
            Retry count: <span className="font-medium">{retryCount}</span>
          </p>
          {secondsAgo !== null && (
            <p>
              Last message: <span className="font-medium">{secondsAgo}s ago</span>
            </p>
          )}
          {secondsAgo === null && <p className="text-slate-400">No messages received yet</p>}
        </div>
      )}
    </div>
  )
